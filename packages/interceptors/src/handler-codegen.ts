...
export function generatePolicyEvaluationCode(enforcementMode: string): string {
  ...
  L(`        if (_sfDangerous[i].re.test(rawCmd)) {`);
  L(`          return { block: enforce, reason: "Dangerous [" + _sfDangerous[i].id + "]: " + _sfDangerous[i].label + " -- " + rawCmd.slice(0, 100), id: "dangerous_commands" };`);
  L(`        }`);
  L(`      }`);
  L(``);
  L(`      // Secrets in shell commands`);
  L(`      for (var s = 0; s < _sfSecrets.length; s++) {`);
  L(`        if (_sfSecrets[s].re.test(rawCmd)) {`);
  L(`          return { block: enforce, reason: "Secret detected [" + _sfSecrets[s].id + "]: " + _sfSecrets[s].label, id: "secrets_leak" };`);
  L(`        }`);
  L(`      }`);
  L(`    }`);
  L(`  }`);
  ...
  L(`          return { block: enforce, reason: "Write to sensitive path [" + _sfSensitivePaths[w].id + "]: " + _sfSensitivePaths[w].label, id: "file_write" };`);
  L(`        }`);
  L(`      }`);
  L(`    }`);
  L(`  }`);
  ...
  L(`        return { block: enforce, reason: "Secret detected [" + _sfSecrets[k].id + "]: " + _sfSecrets[k].label, id: "secrets_leak" };`);
  L(`      }`);
  L(`    }`);
  L(`  }`);
  L(``);
